...
int main(int argc, char** argv) {
    ...
    std::string path = "/home/yukan/Documents/fengfan_calibration/matlab/";
    std::string img_name = "front.jpg";
    ...
    intrinsic << 1.26308344, 666.64620977, 654.80952909, 640.87238056, 360.0141343;
    Eigen::Vector4d distortion(0.61845852, -0.75764563, 0.01255056, 0.00216982);

    double f = (intrinsic[2] + intrinsic[1])/2 * scale;
    double px = intrinsic[3];
    double py = intrinsic[4];


    cv::Mat map_x = cv::Mat::zeros(img.rows, img.cols, CV_32FC1);
    cv::Mat map_y = cv::Mat::zeros(img.rows, img.cols, CV_32FC1);

    for(int i = 0; i < img.rows; i++){
        for(int j = 0; j < img.cols; j++){
            Eigen::Vector3d bearing(j - px, i - py, f);
            bearing.normalize();
            Eigen::Vector2d imPt = worldToCam(bearing, intrinsic, distortion);

            map_x.at<float>(i,j) = imPt[0];
            map_y.at<float>(i,j) = imPt[1];
        }
    }
    ...
    cv::waitKey(0);
//    cv::imwrite("/home/yukan/Documents/apriltag_detection/undistorted.jpg", undistorted);
    return 0;
}
